...
- [ ] **Incentive file attachment on broadcasts** — mirror the capture-page incentive flow so artists can attach an MP3/PDF/ZIP to a broadcast (e.g. "here's the acoustic version of my new single"). New columns on `broadcasts` (`incentive_file_path`, `incentive_file_name`, `incentive_content_type`, `incentive_file_size`), broadcast-scoped download token reuse, UI upload section inside compose dialog, storage counts against tier cap same as capture pages.
- [ ] **Per-broadcast theme editor** — today broadcasts inherit theme from the most-recently-updated capture page via `toEmailTheme()`. Add a `ThemeEditor` in the compose dialog for per-broadcast override (accent/bg/text/button_style/background_style). Inherited values remain the default; override only when artist opens the editor.
...
- [ ] **Revisit broadcast tier caps** — Solo = 0/mo may make the tier feel worthless without any way to email fans between shows; Tour = 4/mo may be too tight now that the creation-lock UX makes the ceiling visible. Don't adjust until there's real signal — check `broadcasts` table for per-month send counts by tier and monitor 429s on POST `/broadcasts` (see `PRICING.md` for cost analysis).
