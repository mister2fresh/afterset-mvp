...
**Paused emails drill-down shipped (2026-04-14):** Original Phase 3 surfaced paused rows as a banner + popup dialog with no self-serve action. Redesigned to: `GET /api/paused-emails` (optional `?reason=`) joins `pending_emails` → `email_templates`, `broadcasts`, `capture_events → fan_captures` and returns fan email/name, page title snapshot, step/broadcast label, paused-ago. `DELETE /api/paused-emails?reason=<reason>` hard-deletes by reason (gated by `requireActive`). `PausedEmailsCard` on `/settings` (mounted below `PlanCard`, `id="paused-emails"`) groups rows by reason (order: email_cap → tier_locked → no_plan → stale), per-group Dismiss, collapse past 5 rows. "View" deep-links via TanStack Router `validateSearch`: `/pages?open=<pageId>` auto-opens the page editor with sequence expanded; `/emails?broadcast=<id>` auto-opens the broadcast compose dialog in preview mode (both use a `consumedRef` + `navigate({ search: {} })` to single-shot the param). `paused-emails-banner.tsx` stripped of the dialog — now a simple Link to `/settings#paused-emails`. Auto-purge via pg_cron (`20260414000000_paused_emails_auto_purge.sql`, hourly at `:17`): `stale` = 14d, `email_cap | tier_locked | no_plan` = 30d after `skip_reason_at`. Only `stale` rows are truly dead on purge — others resume sending automatically if the artist upgrades before the TTL hits.
