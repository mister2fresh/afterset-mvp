...
- **Safe page deletion:** `capture_events.capture_page_id` is nullable with `ON DELETE SET NULL`. Deleting a capture page preserves all fan data and capture history. Each capture event snapshots the page title at capture time (`page_title` column), so gig context survives page edits and deletions. The captures API falls back to the snapshot when the page no longer exists. RLS on `capture_events` routes through `fan_captures → artists` (not `capture_pages`) to handle null page references.
