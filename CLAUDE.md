...
- **Email templates are plain text** — rendered to HTML at send/preview time via `renderFollowUpHtml()`, no React Email dependency. Emails inherit the capture page's theme (`accent_color`, `bg_color`, `text_color`, `button_style`) via `toEmailTheme()`. Broadcasts use the artist's most recently updated capture page for theming. Auto-detects body text and button text contrast based on background/accent lightness.
