...
- [x] **Broadcast card falsely shows "N suppressed (bounced or unsubscribed)" mid-send** — while status was `sending`, the card computed `recipient_count - sent_count` and labeled the gap as suppressed. Fixed: show "N pending" while `status === 'sending'`, "N suppressed (bounced or unsubscribed)" only once `status === 'sent'`. `web/src/components/broadcast-compose-dialog.tsx` BroadcastCard. (fixed April 14)
...
- [x] **Sequence editor locked steps** ✅ 2026-04-13 — steps beyond `limits.sequenceDepth` render with dashed border + Lock badge; at-limit shows UpgradePrompt for next tier. Updated 2026-04-14: locked steps are now expandable into a read-only editor so downgraded artists can still view/copy/preview their own drafts (inputs `readOnly`, toggles disabled, Delete/Save hidden, Preview kept, lock banner at top with upgrade target)
...
- [x] **Paused emails banner** ✅ 2026-04-13 — `web/src/components/paused-emails-banner.tsx` mounted on dashboard above tabs; details Dialog breaks down by reason. Redesigned 2026-04-14: Dialog dropped; banner now Links to `/settings#paused-emails` → new `PausedEmailsCard` drill-down with grouped rows, per-reason batch Dismiss, deep-links to page/broadcast editor, pg_cron auto-purge (stale=14d, others=30d). Endpoint: `GET/DELETE /api/paused-emails`.
...
### Phase 4 — Inactive Tier (post-trial / no-plan state)

Decided 2026-04-14 mid-Phase-3-QA: post-trial artists must drop to a dead-end "inactive" state, not silently fall through to Solo. Full rationale in memory `project_sprint_5_phase_4_inactive_tier.md`. Read-only dashboard + paused capture pages + held emails; no grace period.

- [x] **Add "inactive" to Tier union + TIER_LIMITS** ✅ 2026-04-14 — `Tier = "solo" | "tour" | "superstar" | "inactive"` in `web/src/lib/types.ts`; all-zero `inactive` row added to `TIER_LIMITS` in `api/src/lib/tier.ts`, `worker/src/tier.ts`, and `web/src/lib/pricing.ts`. New `PurchasableTier = Exclude<Tier, "inactive">` for UI dealing with buyable plans. `TIER_DISPLAY` constrained to purchasable tiers.
- [x] **`getEffectiveTier()` returns "inactive" on expired Solo trial** ✅ 2026-04-14 — Solo + expired `trial_ends_at` → `"inactive"`. Solo without a trial stays Solo. `tier-parity.test.ts` asserts worker slim copy matches.
- [x] **Worker serves branded paused page + rejects captures** ✅ 2026-04-14 — `worker/src/index.ts` fetches artist tier via joined PostgREST query in parallel with R2 (`Promise.all`), serves ~2KB branded "signup paused" HTML (midnight bg, honey-gold social links, status 200, `Cache-Control: no-store`, `X-Frame-Options: DENY`, meta noindex) when inactive. POST `/c/:slug` returns 403 `{ inactive: true }` before any tier-limits access.
- [x] **send-batch `no_plan` bucket** ✅ 2026-04-14 — `partitionByTier()` in `api/src/routes/send-batch.ts` classifies inactive rows into a `no_plan` bucket; releases them back to `pending` with `skip_reason='no_plan'`. Emails resume on upgrade.
- [x] **`requireActive` middleware on all write endpoints** ✅ 2026-04-14 — `api/src/middleware/require-active.ts` returns 403 `{ inactive: true }` on inactive effective tier. Wired per-route (not globally) on every mutation in `capture-pages`, `email-templates`, `broadcasts`, `sms-keywords`, `incentive`, `build`, and `settings`. Preview/availability-check routes intentionally NOT gated — artists can still view.
- [x] **Dev: negative `trialDays` + "Expire trial" button** ✅ 2026-04-14 — `POST /dev/set-tier` schema now `min(-365).max(365)`; negative values set `trial_ends_at` in the past to simulate expiry. `DevTierSwitcher` in `plan-card.tsx` adds "Expire trial" button posting `{ tier: "solo", trialDays: -1 }` alongside the existing "Start 30d Tour trial".
- [x] **Usage endpoint + meters accept `no_plan`** ✅ 2026-04-14 — `paused_by_reason.no_plan` flows through `GET /api/usage` → `UsageMeters` tooltip + `PausedEmailsBanner` breakdown.
- [x] **PlanCard inactive state** ✅ 2026-04-14 — `web/src/components/plan-card.tsx` detects `effectiveTier === "inactive"`: destructive "No active plan" badge, hides price + usage meters, red-styled CTA container with "Start a subscription — hello@afterset.net", hides compliance footnote. TierComparison still rendered so artists see what to buy.
- [x] **Dashboard inactive banner** ✅ 2026-04-14 — `web/src/components/inactive-banner.tsx` renders above `<Outlet />` in `_authenticated.tsx` on all authenticated routes; red warning + "Start subscription" link to `/settings`.
- [x] **Capture pages list paused indicator** ✅ 2026-04-14 — `pages.tsx`: "New Page" button replaced with disabled button, per-card status badge shows destructive "Paused" (overriding Active/Inactive), inline title edit and Delete menu item disabled. Data still fully viewable.
- [ ] **Trial countdown warning emails (7d/3d/1d + expiry)** — Deferred. Stripe lifecycle events will plug in here post-MVP. For now, the in-app `TrialCountdown` on PlanCard + dashboard trial badge are the only signal.
